...
def process_subset(position_set_subset, process_id):
    ...
    percentile_pentad_subset = np.empty((len(position_set_subset), 20, 365//5)) ########### 这个函数创建了一个新的数组，其形状由参数指定，但不会初始化数组元素，只是分配了数组的内存空间。
    ####################len(position_set_subset): 这是position_set_subset的长度，可能是一个包含位置的列表或数组。
    ##############################20: 这是数组的第二维。看起来你正在为20个元素分配空间。
    #################################365//5: 这是数组的第三维。它可能计算一年中的"pentads"（5天一组的周期）的数量。假设一年有365天，这个表达式计算了一年中5天一组的周期数。
    ...
    for i, (lat, lon) in tqdm(enumerate(position_set_subset), desc="position", total=len(position_set_subset)):
        ...
        position_data = data[:, :, position_index[:, 0], position_index[:, 1]] #shape (20, 365, 121, 241)这个是维度(年，天，纬度，经度)
        position_data = position_data.reshape(-1, 4) #### reshape 函数中的 -1 参数是一个未知维度的占位符。当在一个维度中使用 -1 时，NumPy 根据数组中的总元素数量和其他维度的大小自动计算该维度的大小。

        moisture_bilinear_interpolation = np.empty((position_data.shape[0]))
        for k, position_data_daily in tqdm(enumerate(position_data), desc="daily", total=position_data.shape[0], leave=False):
            ...
            moisture_bilinear_interpolation[k] = interpolated_data
        moisture_bilinear_interpolation = moisture_bilinear_interpolation.reshape(20, 365)
        moisture_bilinear_interpolation_pentad = np.empty((20, 365//5))
        for pentad_day in range(365//5):
        ...
        percentile_pentad_subset[i] = percentile_pentad
    # save the percentile_pentad_subset and position_set_subset
    np.savez(f"./location_result/percentile_pentad_subset_{process_id}.npz", percentile_pentad_subset=percentile_pentad_subset, position_set_subset=position_set_subset)



final_data = np.empty((len(position_set), 20, 365//5))
