...
def process_subset(position_set_subset, process_id):
    ...
    tp_subset = np.empty((len(position_set_subset), 20, 365)) ########### 这个函数创建了一个新的数组，其形状由参数指定，但不会初始化数组元素，只是分配了数组的内存空间。
    ...
    for i, (lat, lon) in tqdm(enumerate(position_set_subset), desc="position", total=len(position_set_subset)):
        ...
        tp_subset[i] = tp_bilinear_interpolation
    # save the percentile_pentad_subset and position_set_subset
    np.savez(f"./location_result/tp_subset_{process_id}.npz", tp_subset=tp_subset, position_set_subset=position_set_subset)
